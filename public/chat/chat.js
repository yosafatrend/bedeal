...
document.getElementById("ReciverId_txt").addEventListener("input", chatSection);
...
function GetChatHistory(receiver_id){
				$.ajax({
						  ...
						  {
  							$('#dumppy').html(data);
  							var s = document.getElementById("ReciverId_txt");
							s.addEventListener("input", ScrollDown);	 
						  },
						  error: function (jqXHR, status, err) {
 							 // alert('Local error callback');
						  }
